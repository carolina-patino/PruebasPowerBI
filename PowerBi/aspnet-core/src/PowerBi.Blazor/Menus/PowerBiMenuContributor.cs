...
namespace PowerBi.Blazor.Menus;

public class PowerBiMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public PowerBiMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<PowerBiResource>();
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                PowerBiMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(PowerBiPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                PowerBiMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(PowerBiPermissions.Dashboard.Tenant)
        );
